  ...
  methods: {
    ...
    async userLogin () {
      try {
        ...
        // 用户没有登录就往一些页面跳转，会在router里被导航守卫阻止；然后给其加上跳往登录页的链接，但是登录成功后，重定向至之前要跳转的页面。
        // 这里的逻辑就是，检查路由当中是否包含query参数的redirect属性；有：调到query参数指定路由，没有：跳到home
