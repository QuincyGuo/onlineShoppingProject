<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail"
           v-for="(order) in userOrderInfo.detailArrayList"
           :key="order.skuId">
        <h5>商品清单</h5>
    ...
    <div class="money clearFix">
      <ul>
        <li>
          <b><i>{{userOrderInfo.totalNum}}</i>件商品，总商品金额</b>
          <span>¥{{userOrderInfo.totalAmount}}.00</span>
        </li>
        <li>
          <b>返现：</b>
          <span>0.00</span>
    ...
    <div class="trade">
      <div class="price">应付金额:<span>¥{{userOrderInfo.totalAmount}}.00</span></div>
...
export default {
  ...
  data () {
    return {
      msg: '',      // 买家留言
      orderId: '',  // 提交订单后服务器返回的订单号
    }
  },
  mounted () {
    this.$store.dispatch('getUserAddress'); // 获取用户地址
    this.$store.dispatch('getOrderInfo');   // 获取商品清单
  },

  computed: {
    ...mapState({
      userAddress: state => state.trade.userAddress,
      userOrderInfo: state => state.trade.orderInfo,
    }),
  ...
  methods: {
    ...
    async submitOrder () {
      ...
      let data = {
        ...
        orderDetailList: this.userOrderInfo.detailArrayList,    // 最终要购买的商品数据数组，数组每一项内容都是商品名称、价格等数据
      }
      // 2、发送请求
